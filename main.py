...
import config
import layout
import VideoEditingFunctions as vef
import utility


window = layout.sg.Window("Video Compressor", layout.layout)
...
while True:
    ...
    if event == layout.sg.WIN_CLOSED or event == 'Exit':
        ...
        if len(inputFilepathsListed) > 0:
            ...
            entireRuntimeStart = time.time()
            currentRunTimeWatch = time.time()
            if config.trialMinSeconds > 0:
                stepProgressSeconds = int(config.trialMinSeconds * config.averageProcessTimePerMin * config.averageEndTimePerInputMin / 100)
            else:
                stepProgressSeconds = int(vef.VideoFileClip(inputFilepathsListed[0]).duration * config.averageProcessTimePerMin * config.averageEndTimePerInputMin / 100)
